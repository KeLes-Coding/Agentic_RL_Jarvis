...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        print("!!!!!!!!!! 🚀 FIT METHOD STARTED (CCAPO v3.0 Debug Mode) 🚀 !!!!!!!!!!")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Pop additional keys
                for k in ["multi_modal_data", "raw_prompt", "tools_kwargs", "ground_truth_answer"]:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        # (REMAX Logic Omitted for brevity, keep original if needed)
                        pass 
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.actor_rollout_ref.actor.get('use_invalid_action_penalty', True):
                            batch, invalid_metrics = apply_invalid_action_penalty(batch, self.config.actor_rollout_ref.actor.invalid_action_penalty_coef)
                        ...
                        # [CCAPO v3.0] LASR Implementation
                        # ---------------------------------------------------------------------
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.CCAPO:
                            try:
                                infos = batch.non_tensor_batch.get('infos', [])
                                
                                # 安全解包：infos 可能是 np.ndarray (object)
                                ...
                                for idx, info in enumerate(infos):
                                    # 兼容性读取：可能是在 ccapo_trajectory 里，也可能是直接的 won
                                    is_succ = False
                                    if isinstance(info, dict):
                                        if 'ccapo_trajectory' in info:
                                            is_succ = info['ccapo_trajectory']['metrics']['is_success']
                                        else:
                                            is_succ = bool(info.get('won', False))
                                    if is_succ:
                                ...
                                if len(success_indices) > 0:
                                    ...
                                    succ_idx_tensor = torch.tensor(success_indices, device=seq_lens.device)
                                    success_lens = seq_lens[succ_idx_tensor]
                                    
                                    mu_len = torch.mean(success_lens)
                                    std_len = torch.std(success_lens)
                                    if torch.isnan(std_len) or std_len < 1e-6:
                                        std_len = torch.tensor(1.0, device=seq_lens.device)
                                        
                                    # Z-score & Tanh Reweighting
                                    z_scores = (mu_len - success_lens) / (std_len + 1e-6)
                                    beta = 1.0
                                    weights_update = 1.0 + beta * torch.tanh(z_scores)
                                    
                                    # 初始化权重全为 1.0 (Failure Invariance)
                                    lasr_weights = torch.ones(batch.batch.batch_size[0], 1, device=batch.batch['advantages'].device)
                                    # 更新成功者权重
                                    lasr_weights[succ_idx_tensor, 0] = weights_update
                                    
                                    # 应用到 advantages
                                    batch.batch['advantages'] = batch.batch['advantages'] * lasr_weights
                                    
                                    # Log
                                    metrics.update({
                                        "lasr/success_cnt": len(success_indices),
                                        "lasr/mu_len": mu_len.item(),
                                        "lasr/max_weight": weights_update.max().item(),
                                        "lasr/min_weight": weights_update.min().item()
                                    })
                                else:
                                    # 记录 0 以便 inspector 知道我们尝试过
                                    metrics.update({"lasr/success_cnt": 0})
                                    
                            except Exception as e:
                                print(f"⚠️ [CCAPO] LASR Error: {e}")
                                import traceback
                                traceback.print_exc()
                        # ---------------------------------------------------------------------
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            self._dump_generations(inputs, outputs, scores, reward_extra_infos_dict, rollout_data_dir)
                ...
                # Debug Print for Tokens (Simplified)
                try:
                    total_tokens = batch.batch["attention_mask"].sum().item()
                    progress_bar.write(f"Step {self.global_steps} | Tokens: {total_tokens} | Success: {metrics.get('lasr/success_cnt', 'N/A')}")
                except: pass
